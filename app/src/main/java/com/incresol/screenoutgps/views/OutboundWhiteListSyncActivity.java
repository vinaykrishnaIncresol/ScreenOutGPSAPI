...
public class OutboundWhiteListSyncActivity extends AppCompatActivity {
    ...
    public static List<String> outboundCallList_ALL;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_outbound_white_list_sync);
        outboundCallList_ALL = new ArrayList<>();
        ...
        txt_title.setText("Outbound Whitelisted Calls");
        ...
        coordinate_activity_notifications = (CoordinatorLayout) findViewById(R.id.coordinate_activity_outboundwhitelist);
        recyclerView_Users = (RecyclerView) findViewById(R.id.recycler_outbound_usercalls);
        recyclerView_Users.setHasFixedSize(true);
        recyclerView_Company = (RecyclerView) findViewById(R.id.recycler_outbound_companycalls);
        recyclerView_Company.setHasFixedSize(true);
        recyclerView_Location = (RecyclerView) findViewById(R.id.recycler_outbound_locationcalls);
        recyclerView_Location.setHasFixedSize(true);
        recyclerView_Group = (RecyclerView) findViewById(R.id.recycler_outbound_groupcalls);
        ...
        adapter_Users = new BoundCallsAdapter(boundcallsArrayList_Users, OutboundWhiteListSyncActivity.this);
        adapter_Company = new BoundCallsAdapter(boundcallsArrayList_Company, OutboundWhiteListSyncActivity.this);
        adapter_Location = new BoundCallsAdapter(boundcallsArrayList_Location, OutboundWhiteListSyncActivity.this);
        adapter_Group = new BoundCallsAdapter(boundcallsArrayList_Group, OutboundWhiteListSyncActivity.this);
        recyclerView_Users.addOnItemTouchListener(new RecyclerItemClickListener(OutboundWhiteListSyncActivity.this, new RecyclerItemClickListener.OnItemClickListener() {
            @Override
...
//                Toast.makeText(getApplicationContext(), "" + boundcallsArrayList_Users.get(position).getName(), Toast.LENGTH_SHORT).show();
        ...
        recyclerView_Company.addOnItemTouchListener(new RecyclerItemClickListener(OutboundWhiteListSyncActivity.this, new RecyclerItemClickListener.OnItemClickListener() {
            @Override
            public void onItemClick(View view, int position) {
...
//                Toast.makeText(getApplicationContext(), "" + boundcallsArrayList_Company.get(position).getName(), Toast.LENGTH_SHORT).show();
        ...
        recyclerView_Location.addOnItemTouchListener(new RecyclerItemClickListener(OutboundWhiteListSyncActivity.this, new RecyclerItemClickListener.OnItemClickListener() {
            @Override
            public void onItemClick(View view, int position) {
...
//                Toast.makeText(getApplicationContext(), "" + boundcallsArrayList_Location.get(position).getName(), Toast.LENGTH_SHORT).show();
        ...
        recyclerView_Group.addOnItemTouchListener(new RecyclerItemClickListener(OutboundWhiteListSyncActivity.this, new RecyclerItemClickListener.OnItemClickListener() {
            @Override
            public void onItemClick(View view, int position) {
...
//                Toast.makeText(getApplicationContext(), "" + boundcallsArrayList_Group.get(position).getName(), Toast.LENGTH_SHORT).show();
        ...
        if (UtilsClass.isInternetConnected(OutboundWhiteListSyncActivity.this)) {
            new FetchOutboundCalls().execute();
        } else {
            Toast.makeText(OutboundWhiteListSyncActivity.this, "No Internet Connectivity to Update the List of Contacts", Toast.LENGTH_SHORT).show();
        }
    }

    private class FetchOutboundCalls extends AsyncTask<Void, Void, Void> {
        ProgressDialog progressDialog;

        @Override
        protected void onPreExecute() {
            progressDialog = new ProgressDialog(OutboundWhiteListSyncActivity.this);
        ...
        protected Void doInBackground(Void... params) {
            try {
                ...
                final String outboundUrl = UtilsClass.outboundWhitelist + apik;
                AsyncHttpClient client = new SyncHttpClient();
                client.get(outboundUrl, new JsonHttpResponseHandler() {
                    @Override
...
// Users Calls Parsing
                                        ...
                                        try {
                                            dataObjectArray_Users = dataObject.getJSONArray("outbound_user_phone");
                                        } catch (Exception e) {
                                            e.printStackTrace();
                                        }
...
// Comapany Calls Parsing
                                        ...
                                        try {
                                            dataObjectArray_Company = dataObject.getJSONArray("outbound_company_phone");
                                        } catch (Exception e) {
                                            e.printStackTrace();
                                        }
...
// Location Calls Parsing
                                        ...
                                        try {
                                            dataObjectArray_Location = dataObject.getJSONArray("outbound_location_phone");
                                        } catch (Exception e) {
                                            e.printStackTrace();
                                        }
...
// Group Calls Parsing
                                        ...
                                        try {
                                            dataObjectArray_Group = dataObject.getJSONArray("outbound_group_phone");
                                        } catch (Exception e) {
                                            e.printStackTrace();
                                        }
                                        ...
                                        if (outboundCallList_ALL.size() > 0) {
                                            outboundCallList_ALL.clear();
                                        }
                                        ...
                                        if (boundcallsArrayList_Users.size() > 0) {
                                            for (int i = 0; i < boundcallsArrayList_Users.size(); i++) {
                                                ...
                                                outboundCallList_ALL.add(bndCall.getNumber());
                                            }
                                        }
                                        ...
                                        if (outboundCallList_ALL != null) {
                                            SharedPreferences sharedPreferences1 = getSharedPreferences("MYPREFERENCES", Context.MODE_PRIVATE);
                                            SharedPreferences.Editor editor = sharedPreferences1.edit();
                                            editor.putInt("outboundCallList_Count", outboundCallList_ALL.size());
                                            for (int i = 0; i < outboundCallList_ALL.size(); i++) {
                                                System.out.println("$$$$ size -> " + outboundCallList_ALL.size() + " of " + i + "number -> " + outboundCallList_ALL.get(i));
                                                editor.putString("outbound_" + i, outboundCallList_ALL.get(i));
                                            }
                                            editor.commit();
                                        }
                                    } else {
                                        System.out.println("Error on Fetching outboundContactsList!");
                                    }
...
//                            super.onFailure(statusCode, headers, responseString, throwable);
                        System.out.println("Fetching outboundCalls List got Failure !!!!");
                    }
                });
            } catch (Exception e) {
                e.printStackTrace();
